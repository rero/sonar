...
def test_create(document_json):
    ...
    assert DocumentRecord.get_record_by_pid(record["pid"])["pid"] == record["pid"]
...
def test_get_pid_by_ref_link(document):
    ...
    link = url_for("invenio_records_rest.doc_item", _external=True, pid_value="10000")
...
def test_get_record_by_bucket(app, db, document_with_file):
    ...
    db.session.commit()
    pid = PersistentIdentifier.get("oai", "oai:sonar.ch:" + document_with_file["pid"])
    db.session.delete(pid)
    db.session.commit()
    assert not SonarRecord.get_record_by_bucket(document_with_file["_bucket"])
