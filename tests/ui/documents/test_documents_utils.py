    ...
    assert utils.publication_statement_text(
        {
            ...
            "statement": [
                ...
                {"label": [{"value": "1900-1999"}], "type": "Date"},
            ],
        }
...
def test_get_file_restriction(app, organisation, admin, monkeypatch, embargo_date):
    ...
    with app.test_request_context(environ_base={"REMOTE_ADDR": "127.0.0.1"}) as req:
        ...
        assert utils.get_file_restriction(
            {"access": "coar:c_f1cf", "embargo_date": embargo_date.isoformat()},
            [organisation],
        ) == {"date": embargo_date.strftime("%d/%m/%Y"), "restricted": True}
...
def test_get_thumbnail():
    ...
    document = {
        "pid": 1,
        "external_url": True,
        "_files": [{"key": "test.pdf"}, {"key": "test-pdf.jpg"}],
    }
    file = {"key": "test.pdf", "restriction": {"restricted": True}}

    # File is restricted
    assert utils.get_thumbnail(file, document) == "static/images/restricted.png"

    # Thumbnail is returned
    file["restriction"]["restricted"] = False
    assert utils.get_thumbnail(file, document) == "/documents/1/files/test-pdf.jpg"
