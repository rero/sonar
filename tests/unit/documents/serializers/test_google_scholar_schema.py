...
def test_title(minimal_document):
    """Test name."""
    result = google_scholar_v1.transform_record(minimal_document["pid"], minimal_document)
...
def test_keywords(minimal_document):
    ...
    result = google_scholar_v1.transform_record(minimal_document["pid"], minimal_document)
    assert '<meta name="citation_keywords" content="Subject 1 ; Subject 2 ; Sujet 1 ; Sujet 2">' in result
...
def test_pdf_url(minimal_document):
    ...
    result = google_scholar_v1.transform_record(minimal_document["pid"], minimal_document)
    assert '<meta name="citation_pdf_url" content="http://localhost/documents/1000/files/test.pdf">' in result
    ...
    result = google_scholar_v1.transform_record(minimal_document["pid"], minimal_document)
    assert '<meta name="citation_pdf_url" content="https://some.domain/file.pdf">' in result
...
def test_abstract_html_url(minimal_document):
    ...
    result = google_scholar_v1.transform_record(minimal_document["pid"], minimal_document)
    assert '<meta name="citation_abstract_html_url" content="http://localhost/global/documents/1000">' in result
...
def test_volume(app, minimal_document):
    ...
    # Only one page
    minimal_document["partOf"] = [{"document": {"title": "Host document"}, "numberingVolume": "1"}]
    result = google_scholar_v1.transform_record(minimal_document["pid"], minimal_document)
