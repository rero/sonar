...
def test_list(app, client, make_deposit, superuser, admin, moderator, submitter, user):
...
def test_read(
    ...
    moderator["subdivision"] = {"$ref": f"https://sonar.ch/api/subdivisions/{sub_pid}"}
    moderator.commit()
    moderator.reindex()
    db.session.commit()
    res = client.get(url_for("invenio_records_rest.depo_item", pid_value=deposit1["pid"]))
...
def test_update(client, make_deposit, superuser, admin, moderator, submitter, user):
...
def test_delete(client, db, make_deposit, superuser, admin, moderator, submitter, user):
