...
def test_list(app, client, make_user, superuser, admin, moderator, submitter, user):
...
def test_create(client, organisation, superuser, admin, moderator, submitter, user):
...
def test_read(client, make_user, superuser, admin, moderator, submitter, user):
    ...
    # Not logged
    res = client.get(url_for("invenio_records_rest.user_item", pid_value=user["pid"]))
    assert res.status_code == 401
...
def test_update(client, make_user, superuser, admin, moderator, submitter, user):
