...
class Update(AdminAction):
    ...
    def excludes(self, record=None, **kwargs):
        ...
        if record["metadata"].get("validation", {}).get("status") == Status.REJECTED:
            return [any_user]
        ...
        if current_user_record.is_moderator:
            ...
            return [any_user] if current_organisation["pid"] != organisation_pid else []

        # For submitters, they can only modify their own projects
        if current_user_record.is_submitter:
            user_pid = (
                SonarRecord.get_pid_by_ref_link(record["metadata"]["user"]["$ref"])
                if record["metadata"]["user"].get("$ref")
                else record["metadata"]["user"]["pid"]
            )
