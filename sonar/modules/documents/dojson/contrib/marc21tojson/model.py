...
def get_mef_person_link(person_id, key, value):
    """Get mef person link."""
    ...
    if person_id:
        ...
        if request.status_code == requests.codes.ok:
            ...
            print(
                "ERROR: MEF request", url, request.status_code, file=sys.stderr
            )
    return mef_url
...
def marc21_to_abstracts(self, key, value):
    ...
    """
    if not self.get("abstracts", None):
        self["abstracts"] = {}

    self["abstracts"][value.get("9")] = value.get("a")
    return None
...
def marc21_to_subjects(self, key, value):
    ...
    """
    if not self.get("subjects", None):
        self["subjects"] = {}

    self["subjects"][value.get("9")] = value.get("a").split(" ; ")
    return None
