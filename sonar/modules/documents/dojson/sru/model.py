...
def marc21_to_identified_by_from_001(self, key, value):
    ...
    identified_by.append({"type": "bf:Local", "source": "swisscovery", "value": value})
...
def marc21_to_document_type_from_leader(self, key, value):
    ...
    def determine_type(field):
        ...
        # Doctoral thesis
        if (
            "dissertation" in field.get("a", "").lower()
            or "dissertation" in field.get("b", "").lower()
            or "thèse" in field.get("a", "").lower()
...
def marc21_to_provision_activity_from_264_3(self, key, value):
    ...
    for place in utils.force_list(value.get("a", [])):
        manufacture["statement"].append({"type": "bf:Place", "label": {"value": place}})
