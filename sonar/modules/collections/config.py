...
class Configuration:
    ...
    rest_endpoint = {
        "pid_type": PID_TYPE,
        "pid_minter": f"{RESOURCE_NAME}_id",
        "pid_fetcher": f"{RESOURCE_NAME}_id",
        "default_endpoint_prefix": True,
        "record_class": f"{MODULE_PATH}.api:Record",
        "search_class": f"{MODULE_PATH}.api:RecordSearch",
        "indexer_class": f"{MODULE_PATH}.api:RecordIndexer",
        "search_index": RESOURCE_NAME,
        "record_serializers": {
            "application/json": (f"{MODULE_PATH}.serializers" ":json_v1_response"),
        },
        "search_serializers": {
            "application/json": (f"{MODULE_PATH}.serializers" ":json_v1_search"),
        },
        "record_loaders": {
            "application/json": (f"{MODULE_PATH}.loaders" ":json_v1"),
        },
        "list_route": f"/{RESOURCE_NAME}/",
        "item_route": f'/{RESOURCE_NAME}/<pid({PID_TYPE}, record_class="{MODULE_PATH}.api:Record"):pid_value>',
        "default_media_type": "application/json",
        "max_result_window": 10000,
        "search_factory_imp": f"{MODULE_PATH}.query:search_factory",
